...
?>
 <?php
	...
	$id=$_GET['id'];
	$query=mysqli_query($conn,"select * from attandance where s_id='$id'");
...
<body>
	...
	<table id="mainbodytable">
		...
		<tr>
			<td>
		<form style="margin:30px;"method="POST" action="updateattandance.php?id=<?php echo $id; ?>">
		<label>no of classes:</label><input type="number" value="<?php echo $row['noc']; ?>" name="noc">
		<label>classes attended:</label><input type="number" value="<?php echo $row['ca']; ?>" name="ca">
        <label>percentage:</label><input type="number" value="$percentage = ($portion / $total) * 100;" name="percentage">
...
<?php
 function cal_percentage($ca, $noc) {
  $count1 = $ca/ $noc;
  $count2 = $count1 * 100;
